...
processAll <- function() {
    ...
    groupNumbers <- 1:4
    elecNumbers <- 1:64
    resultsFilenamePattern <- "results/EC2_B76/resultsWav%d%d.RData"
    erpFigFilenamePattern <- "figures/EC2_B76/erp%d.eps"
    itcFigFilenamePattern <- "figures/EC2_B76/itc%d.eps"
    erspFigFilenamePattern <- "figures/EC2_B76/ersp%d.eps"
    xlim <- c(-.5, .6)
    ylim <- c(2, 300)
    # ylim <- NULL
    # zlim <- c(0, 1)
