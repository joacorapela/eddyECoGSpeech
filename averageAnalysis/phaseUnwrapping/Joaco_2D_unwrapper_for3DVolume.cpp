...
printf("width=%d, height=%d, depth=%d\n", volume_width, volume_height, volume_depth);
	...
	int No_of_Edges = (volume_width)*(volume_height-1) + (volume_width-1)*(volume_height);

	float *WrappedImage, *UnwrappedImage;
	int image_size = volume_height * volume_width;
...
//     int i = 1024;
     ...
     for(i=0; i<volume_depth; i++) {
        ...
        WrappedImage = WrappedVolume+i*image_size;
	    //initialise the pixels
	    initialisePIXELs(WrappedImage, pixel, volume_width, volume_height);

	    calculate_reliability(WrappedImage, pixel, volume_width, volume_height);

	    horizentalEDGEs(pixel, edge, volume_width, volume_height);

	    verticalEDGEs(pixel, edge, volume_width, volume_height);
...
// printf("About to enter quicker_sort\n");
// if(i==1024) {
//     printf("At breakpoint\n");
// }
	    quicker_sort(edge, edge + No_of_Edges - 1);
// printf("Exited quicker_sort\n");
	
	    //gather PIXELs into groups
	    gatherPIXELs(edge, volume_width, volume_height);


	    //unwrap the whole image
	    unwrapImage(pixel, volume_width, volume_height);
	    //copy the image from PIXEL structure to the wrapped phase array passed to this function
	    returnImage(pixel, UnwrappedImage, volume_width, volume_height);
int j = 0;
printf("UnwrappedImage_%d\n", i);
for(j=0; j<16; j++) {
    printf("%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n", UnwrappedImage[j*16+0], UnwrappedImage[j*16+1], UnwrappedImage[j*16+2], UnwrappedImage[j*16+3], UnwrappedImage[j*16+4], UnwrappedImage[j*16+5], UnwrappedImage[j*16+6], UnwrappedImage[j*16+7], UnwrappedImage[j*16+8], UnwrappedImage[j*16+9], UnwrappedImage[j*16+10], UnwrappedImage[j*16+11], UnwrappedImage[j*16+12], UnwrappedImage[j*16+13], UnwrappedImage[j*16+14], UnwrappedImage[j*16+15]);
}
        memcpy(UnwrappedVolume+i*image_size, UnwrappedImage, image_size*sizeof(float)) ;
	    // write_data(unwrapped_image_name,UnwrappedImage,image_size);
printf("UnwrappedVolume for image %d\n", i);
for(j=0; j<16; j++) {
    printf("%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n", UnwrappedVolume[i*image_size+j*16+0], UnwrappedVolume[i*image_size+j*16+1], UnwrappedVolume[i*image_size+j*16+2], UnwrappedVolume[i*image_size+j*16+3], UnwrappedVolume[i*image_size+j*16+4], UnwrappedVolume[i*image_size+j*16+5], UnwrappedVolume[i*image_size+j*16+6], UnwrappedVolume[i*image_size+j*16+7], UnwrappedVolume[i*image_size+j*16+8], UnwrappedVolume[i*image_size+j*16+9], UnwrappedVolume[i*image_size+j*16+10], UnwrappedVolume[i*image_size+j*16+11], UnwrappedVolume[i*image_size+j*16+12], UnwrappedVolume[i*image_size+j*16+13], UnwrappedVolume[i*image_size+j*16+14], UnwrappedVolume[i*image_size+j*16+15]);
}
/*
int row=0;
int col=0;
printf("UnwrappedImage[%d,%d]=%f\n", row, col, UnwrappedImage[row*volume_width+col]);
printf("UnwrappedVolume[%d,%d,%d]=%f\n", row, col, i, UnwrappedVolume[row+col*volume_width+i*image_size]);
getchar();
*/
    }
	free(edge);
