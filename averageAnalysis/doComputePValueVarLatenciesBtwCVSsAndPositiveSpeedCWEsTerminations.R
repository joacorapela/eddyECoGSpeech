...
processAll <- function() {
    ...
    resultsFilenamePattern <-
     "results/%s/pValueVarLatenciesBtwCVSsAndPositiveSpeedCWEsTerminationsSign%.02f.RData"
    figFilenamePattern <-
     "figures/%s/histVarLatenciesBtwCVSsAndPositiveSpeedCWEsTerminationsSign%.02f.eps"
    ...
    res <- resampleStatNullLatenciesBtwCVSsAndCWEsPhase(
            stat=var, infoInit=infoInit, infoTerm=infoTerm, cwes=cwes, 
