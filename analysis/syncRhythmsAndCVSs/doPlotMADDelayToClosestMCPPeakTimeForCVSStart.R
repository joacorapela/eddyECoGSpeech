...
#     xlim <- c(500, 600)
    ...
    figFilenamePattern <- "figures/%s/madDelayToClosestMCPPeakForCVSStart%.02ffpsFromFreq%.02fToFreq%.02fFromTime%03dToTime%03d_squareXMin%02dXMax%02dYMin%02dYMax%02d_plotFromTime%03dToTime%03d_medianWindowSize%d.eps"
    ...
    mcpPeaksInfo <- getClosestMCPPeakInfoForCVSs(meansCosPhaseInfo=mcpInfo, cvsStartTimes=resEpochsSamples$times)

    xlab <- "Time (sec)"
    ylab <- expression("mad(CVS Start Time - " ~ bar(cos(phi)) ~ "Peak Time)")
    filteredDelays <- runmad(x=-mcpPeaksInfo$delaysMCPPeakAndCVS, k=medianWindowSize)
