...
processAll <- function() {
    ...
    resultsFilenamePattern <- "results/%s/phasesAtCVSPresentationsElec%03d-%03dFrom%03dTo%03dHtFilteredFrom%.02fTo%0.2fOrder%02d.RData"

    toTime <- fromTime + duration
    resultsFilename <- 
     sprintf(resultsFilenamePattern, sessionLabel, min(elecNumbers), max(elecNumbers), fromTime, toTime, lowCutoff, highCutoff, order)
