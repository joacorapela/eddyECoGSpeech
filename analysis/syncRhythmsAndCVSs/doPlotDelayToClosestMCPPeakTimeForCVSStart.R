...
#     xlim <- c(500, 600)
    ...
    figFilenamePattern <- "figures/%s/delayToClosestMCPPeakForCVSStart%.02ffpsFromFreq%.02fToFreq%.02fFromTime%03dToTime%03d_squareXMin%02dXMax%02dYMin%02dYMax%02d_plotFromTime%03dToTime%03d_medianWindowSize%d.eps"
    ...
    xlab <- "Time (sec)"
    ylab <- expression("CVS Start Time - " ~ bar(cos(phi)) ~ "Peak Time (sec)")
    filteredDelays <- filter(MedianFilter(medianWindowSize), -mcpPeaksInfo$delaysMCPPeakAndCVS)
    ylim <- max(abs(filteredDelays))*c(-1,1)
    ...
    p <- p + xlim(xlim)
    p <- p + geom_hline(yintercept=median(-mcpPeaksInfo$delaysMCPPeakAndCVS[mcpPeaksInfo$cvsStartTimes>transientDuration]), color=hlineColor)
    p <- p + ylim(ylim)
    p <- p + xlab(xlab)
    p <- p + ylab(ylab)
