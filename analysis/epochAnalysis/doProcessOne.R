...
processAll <- function() {
    ...
    ecogFilename <- "rData/EC2_B105/RawHTK/Wav347.bin"
...
winTimeIndices <- which(-.5<=res$times & res$times<=.6)
winFreqIndex <- which.min(abs(res$freqs-1/1.62))
winITCValues <- res$itc[winTimeIndices, winFreqIndex]
maxITC <- max(winITCValues)
    browser()
}
