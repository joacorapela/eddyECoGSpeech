...
processAll <- function() {
    ...
    resultsFilenamePattern <-
     "results/%s/pValuesPropOfStartTimesInSilenceOfCWEsWithPosSpeedFilteredFrom%.02fTo%.02fOrder%02dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.%s"
    figFilenamePattern <-
     "figures/%s/pValuesPropOfStartTimesInSilenceOfCWEsWithPosSpeedFilteredFrom%.02fTo%.02fOrder%02dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.eps"
