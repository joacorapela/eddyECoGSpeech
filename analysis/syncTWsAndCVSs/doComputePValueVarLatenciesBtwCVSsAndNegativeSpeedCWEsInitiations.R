...
processAll <- function() {
    ...
    if(cwesWithPositiveSpeed) {
        xlab <- "Variance CVS Start Time - CWE Positive Speed Start Time"
    } else {
        xlab <- "Variance CVS Start Time - CWE Negative Speed Start Time"
    }
    ylab <- "Count"
    ...
    if(cwesWithPositiveSpeed) {
        resultsFilenamePattern <- 
         "results/%s/pValueVarianceLatenciesBtwCVSsAndPositiveSpeedCWEsInitiationsSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.RData"
        figFilenamePattern <-
         "figures/%s/histVarianceLatenciesBtwCVSsAndPositiveSpeedCWEsInitiationsSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.eps"
    } else {
        resultsFilenamePattern <- 
         "results/%s/pValueVarianceLatenciesBtwCVSsAndNegativeSpeedCWEsInitiationsSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.RData"
        figFilenamePattern <-
         "figures/%s/histVarianceLatenciesBtwCVSsAndNegativeSpeedCWEsInitiationsSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.eps"
    }
...
# assign(x="caseBands", value=c(), env=.GlobalEnv)
    res <- resampleStatNullLatenciesBtwCVSsAndCWEsPhase(
            stat=var, infoInit=infoInit, infoTerm=infoTerm, 
