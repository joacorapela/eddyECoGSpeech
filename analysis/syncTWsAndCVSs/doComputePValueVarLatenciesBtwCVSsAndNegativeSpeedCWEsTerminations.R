...
processAll <- function() {
    ...
    if(cwesWithPositiveSpeed) {
        xlab <- "Variance CVS End Time - CWE Positive Speed End Time"
    } else {
        xlab <- "Variance CVS End Time - CWE Negative Speed End Time"
    }
    ylab <- "Count"
    ...
    if(cwesWithPositiveSpeed) {
        resultsFilenamePattern <- 
         "results/%s/pValueVarianceLatenciesBtwCVSsAndPositiveSpeedCWEsTerminationsSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.RData"
        figFilenamePattern <-
         "figures/%s/histVarianceLatenciesBtwCVSsAndPositiveSpeedCWEsTerminationsSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.eps"
    } else {
        resultsFilenamePattern <- 
         "results/%s/pValueVarianceLatenciesBtwCVSsAndNegativeSpeedCWEsTerminationsSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.RData"
        figFilenamePattern <-
         "figures/%s/histVarianceLatenciesBtwCVSsAndNegativeSpeedCWEsTerminationsSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.eps"
    }
