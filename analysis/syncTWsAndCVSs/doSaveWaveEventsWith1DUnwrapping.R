...
processAll <- function() {
    ...
    saveDT <- .1
    unwrapThreshold <- pi
    metaDataPhasesDatacubeFilenamePattern <- "../preProcessing/results/%s/metaDataPhasesDatacubeFilteredFrom%.02fTo%.02fOrder%02dZScored%dFromTime%.02fTo%.02f.%s"
    phasesDatacubeFilenamePattern <- "../preProcessing/results/%s/phasesDatacubeFilteredFrom%.02fTo%.02fOrder%02dZScored%dFromTime%.02fTo%.02f.%s"
    waveEventsFilenamePattern <- "results/%s/waveEventsWith1DPhaseUnwrappingFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.%s"
    ...
    for(i in 1:length(timesToSave)) {
        ...
        unwrappedElectrodesPhases <- 
         unwrap1DPhases(phases=electrodesPhases[sampleToSave,], 
                         threshold=unwrapThreshold)
        phaseDiffs[i,] <- unwrappedElectrodesPhases-unwrappedElectrodesPhases[1]
    }
    ...
    waveEventsRDataFilename <- sprintf(waveEventsFilenamePattern, sessionName, lowCutoff, highCutoff, order, zScore, saveFromTime, saveToTime, saveDT, elecNumbers[1], elecNumbers[length(elecNumbers)], "RData")
    save(wes, file=waveEventsRDataFilename)
    waveEventsCSVFilename <- sprintf(waveEventsFilenamePattern, sessionName, lowCutoff, highCutoff, order, zScore, saveFromTime, saveToTime, saveDT, elecNumbers[1], elecNumbers[length(elecNumbers)], "csv")
