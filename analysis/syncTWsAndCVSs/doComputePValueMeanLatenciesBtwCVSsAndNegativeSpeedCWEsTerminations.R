...
processAll <- function() {
    ...
    if(cwesWithPositiveSpeed) {
        xlab <- "Mean CVS End Time - CWE Positive Speed End Time"
    } else {
        xlab <- "Mean CVS End Time - CWE Negative Speed End Time"
    }
    ylab <- "Count"
    ...
    if(cwesWithPositiveSpeed) {
        resultsFilenamePattern <- 
         "results/%s/pValueMeanLatenciesBtwCVSsAndPositiveSpeedCWEsTerminationsSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.RData"
        figFilenamePattern <-
         "figures/%s/histMeanLatenciesBtwCVSsAndPositiveSpeedCWEsTerminationsSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.eps"
    } else {
        resultsFilenamePattern <- 
         "results/%s/pValueMeanLatenciesBtwCVSsAndNegativeSpeedCWEsTerminationsSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.RData"
        figFilenamePattern <-
         "figures/%s/histMeanLatenciesBtwCVSsAndNegativeSpeedCWEsTerminationsSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.eps"
    }
