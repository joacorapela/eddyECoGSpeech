...
processAll <- function() {
    ...
    waveEventsWith1DPhaseUnwrappingFilename <- sprintf(waveEvenWith1DPhaseUnwrappingtsFilenamePattern, sessionName, lowCutoff, highCutoff, order, saveFromTime, saveToTime, saveDT, elecNumbers[1], elecNumbers[length(elecNumbers)], "RData")
    waveEventsWith2DPhaseUnwrappingFilename <- sprintf(waveEvenWith2DPhaseUnwrappingtsFilenamePattern, sessionName, lowCutoff, highCutoff, order, saveFromTime, saveToTime, saveDT, elecNumbers[1], elecNumbers[length(elecNumbers)], "RData")
    waveEventsWith3DPhaseUnwrappingFilename <- sprintf(waveEvenWith3DPhaseUnwrappingtsFilenamePattern, sessionName, lowCutoff, highCutoff, order, saveFromTime, saveToTime, saveDT, elecNumbers[1], elecNumbers[length(elecNumbers)], "RData")

    waveEventsWithoutPhaseUnwrapping <- get(load(waveEventsWithoutPhaseUnwrappingFilename))
    waveEventsWithoutPhaseUnwrapping$speeds[waveEventsWithoutPhaseUnwrapping$pValues>significance|abs(waveEventsWithoutPhaseUnwrapping$r)<rThreshold] <- NA
    waveEventsWith1DPhaseUnwrapping <- get(load(waveEventsWith1DPhaseUnwrappingFilename))
    waveEventsWith1DPhaseUnwrapping$speeds[waveEventsWith1DPhaseUnwrapping$pValues>significance|abs(waveEventsWith1DPhaseUnwrapping$r)<rThreshold] <- NA
    waveEventsWith2DPhaseUnwrapping <- get(load(waveEventsWith2DPhaseUnwrappingFilename))
    waveEventsWith2DPhaseUnwrapping$speeds[waveEventsWith2DPhaseUnwrapping$pValues>significance|abs(waveEventsWith2DPhaseUnwrapping$r)<rThreshold] <- NA
    waveEventsWith3DPhaseUnwrapping <- get(load(waveEventsWith3DPhaseUnwrappingFilename))
    waveEventsWith3DPhaseUnwrapping$speeds[waveEventsWith3DPhaseUnwrapping$pValues>significance|abs(waveEventsWith3DPhaseUnwrapping$r)<rThreshold] <- NA
