...
processAll <- function() {
    ...
    if(ventroDorsalSpeed) {
        xlab <- "Latency 'end ventro-dorsal CWE' - 'start CVS' (sec)"
    } else {
        xlab <- "Latency 'end dorso-ventral CWE' - 'end CVS' (sec)"
    }
    ...
    if(ventroDorsalSpeed) {
        figFilenamePattern <- 
         "figures/%s/histLatenciesBtwCVSsAndPositiveSpeedCWETerminationsWithRandomizedSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.png"
    } else {
        figFilenamePattern <- 
         "figures/%s/histLatenciesBtwCVSsAndNegativeSpeedCWETermintationsWithRandomizedSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.png"
    }
    ...
    p <- p + scale_fill_manual(name="CVS end order", values=c("blue", "red"),
