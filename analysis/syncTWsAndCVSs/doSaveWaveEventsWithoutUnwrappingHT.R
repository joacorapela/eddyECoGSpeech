...
processAll <- function() {
    ...
    elecNumbers <- 135:142
    distancesFromRefElec <- ((1:length(elecNumbers))-1)*1e-3
    saveFromTime <- 0
    saveToTime <- 700
    saveDT <- .1
    nResamples <- 1000
    htFilenamePattern <- "../preProcessing/results/%s/htFilteredFrom%.02fTo%.02fOrder%02dZScored%dWav%d%d.RData"
    waveEventsFilenamePattern <- "resultsTest/%s/waveEventsWithoutPhaseUnwrappingFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.%s"
