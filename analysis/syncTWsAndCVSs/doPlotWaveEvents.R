...
"results/%s/waveEventsWithoutPhaseUnwrappingFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.%s"
...
#      "results/%s/contiguousWaveEventsWith1DPhaseUnwrappingSignificance%.02frTheshold%.02fFilteredFrom%.02fTo%.02fOrder%02dZScored%dSaveFromTime%.02fToTime%.02fDT%.02fFromElec%03dToElec%03d.%s"
    ...
    significantIndices <- waveEvents$pValue<significance & abs(waveEvents$r)>rThreshold
    ...
    eventsFactor <- factor(c(rep(x="init", times=length(infoInit$time)),
                            ...
                            levels=c("init", "term"))
    eventsDF <- data.frame(time=c(infoInit$time, infoTerm$time),
                            event=eventsFactor)
