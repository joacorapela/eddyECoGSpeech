...
{ 
    if(argc!=5) {
        printf("Usage %s:  <input wrapped image filename> <output unwrapped phase image filename> <width> <height>\n", argv[0]);
        return 1;
    }
	float *WrappedImage, *UnwrappedImage;
    char *wrapped_image_name = argv[1];
    char *unwrapped_image_name = argv[2];
	...
	read_data(wrapped_image_name,WrappedImage,image_size);
